...
{
    if (len > 0)
    { // already stored, no need to access filesystem
        return len;
    }
    if (fs)
    { // fetch file contents
...
{
    if (endStream)
    { // user function provided via ::setStreamEnder( fn )
        endStream(this);
        return;
    }
...
bool esp32FOTA::execOTA()
{
    ...
    { // a data partition was specified in the json manifest, handle the spiffs partition first
        if (_fs)
        { // Possible risk of overwriting certs and signatures, cancel flashing!
...
bool esp32FOTA::execOTA(int partition, bool restart_after)
{
    ...
    if (!canBegin)
    {
        ...
        F_abort();
        if (onUpdateBeginFail)
            onUpdateBeginFail(partition);
        return false;
    }
...
#define CHECK_SIG_ERROR_VALIDATION_FAILED -2
        ...
        if (!_target_partition)
        {
            ...
            if (onUpdateCheckFail)
                onUpdateCheckFail(partition, CHECK_SIG_ERROR_PARTITION_NOT_FOUND);
            delete[] signature;
            return false;
        }
        ...
        if (!validate_sig(_target_partition, signature, updateSize))
        {
            ...
            if (onUpdateCheckFail)
                onUpdateCheckFail(partition, CHECK_SIG_ERROR_VALIDATION_FAILED);
    ...
    if (F_Update.isFinished())
    {
        ...
        if (onUpdateFinished)
            onUpdateFinished(partition, restart_after);
...
std::pair<bool, String> esp32FOTA::checkJSONManifest(JsonVariant doc, int groupNumber)
{
    ...
    if (has_url)
    { // Basic scenario: a complete URL was provided in the JSON manifest, all other keys will be ignored
        _firmwareUrl = doc["url"].as<const char *>();
        if (has_hostname)
        { // If the manifest provides both, warn the user
            log_w("Manifest provides both url and host - Using URL");
        }
    }
    else if (has_firmware && has_hostname && has_port)
    { // Precise scenario: Hostname, Port and Firmware Path were provided
        _firmwareUrl = protocol + "://" + doc["host"].as<const char *>() + ":" + portnum + doc["bin"].as<const char *>();
        if (has_filesystem)
        { // More complex scenario: the manifest also provides a [spiffs, littlefs or fatfs] partition
...
static int64_t getHTTPStream(esp32FOTA *fota, int partition)
{
    ...
    else
    {
        ...
        case 204:
            log_e("Status: 204 (No contents), ");
            break;
        case 401:
            log_e("Status: 401 (Unauthorized), check setExtraHTTPHeader() values");
            break;
        case 403:
            log_e("Status: 403 (Forbidden), check path on webserver?");
            break;
        case 404:
            log_e("Status: 404 (Not Found), also a palindrom, check path in manifest?");
            break;
        case 418:
            log_e("Status: 418 (I'm a teapot), Brit alert!");
            break;
        case 429:
            log_e("Status: 429 (Too many requests), throttle things down?");
            break;
        case 500:
            log_e("Status: 500 (Internal Server Error), you broke the webs!");
            break;
        default:
            // This error may be a false positive or a consequence of the network being disconnected.
            // Since the network is controlled from outside this class, only significant error messages are reported.
