...
#else
__attribute__((unused)) static bool F_canBegin_cb(bool mode_z, int updateSize, int partition)
{ // implement debug here
  return (mode_z ? F_Update.beginz(UPDATE_SIZE_UNKNOWN, partition) : F_Update.begin(updateSize, partition));
}
__attribute__((unused)) static bool F_isZlibStream_cb(Stream *stream, int partition, String flashFileSystemUrl, String firmwareUrl)
{ // implement debug here
...
public:
  ...
  template <typename T>
  void setPubKey(T *asset)
  {
    _cfg.pub_key = (CryptoAsset *)asset;
    _cfg.check_sig = true;
  }
  template <typename T>
  void setRootCA(T *asset)
  {
    _cfg.root_ca = (CryptoAsset *)asset;
    _cfg.unsafe = false;
  }
   ...
   */
  std::pair<OtaCheckStatus, String> execHTTPcheck(int groupNumber = -1);
...
private:
  ...
  String getDeviceID();
  std::pair<bool, String> checkJSONManifest(JsonVariant JSONDocument, int groupNumber);
